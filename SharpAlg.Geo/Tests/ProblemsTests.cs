...
namespace SharpAlg.Geo.Tests {
    ...
    public class ProblemsTests : ExprFunctionalTestsBase {
        protected override Builder CreateBuilder() {
            return Builder.CreateRealLife(openBraces: false);
        }
        [Test, Explicit]
...
", res);
        ...
        public void Perpendicular2_Calc() {
            var res = GetPerpendocularZeroAssertion2(new Line(builder, 0, 1, 4), new Circle(builder, 0, 0, 25));
        ...
        string Perpendicular2_Maple() {
            ...
            var res = GetPerpendocularZeroAssertion2(new Line(builder, 0, 1, Param("b")), new Circle(builder, Param("X0"), Param("Y0"), Param("R")));
            return string.Format(@"
assume(R>0);
simplify({0});
", res);
