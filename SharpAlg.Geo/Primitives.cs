...
namespace SharpAlg.Geo {
    ...
    public class Line {
        ...
        public override string ToString() {
            return CachingBuilder.Simple.Build((A, B, C, x, y) => A * x + B * y + C, A, B, C, Param("x"), Param("y")).ToString();
        }
    }
    ...
    public class Circle {
        ...
        public override string ToString() {
            return CachingBuilder.Simple.Build((X, Y, R, x, y) => ((x - X) ^ 2) + ((y - Y) ^ 2) - R, X, Y, R, Param("x"), Param("y")).ToString();
        }
    }
